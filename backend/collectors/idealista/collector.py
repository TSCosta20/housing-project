...
class IdealistaCollector(Collector):
    source_name = "idealista"

    def __init__(
        self,
        buy_search_url: str | None = None,
        rent_search_url: str | None = None,
        timeout_seconds: float = 20.0,
        max_items: int | None = None,
        max_pages: int | None = None,
    ) -> None:
        self.buy_search_url = buy_search_url or "https://www.idealista.pt/comprar-casas/portugal/"
        self.rent_search_url = rent_search_url or "https://www.idealista.pt/arrendar-casas/portugal/"
        self.timeout_seconds = timeout_seconds
        self.max_items = max_items if max_items is not None else _read_positive_int_env("IDEALISTA_MAX_ITEMS")
        self.max_pages = max_pages if max_pages is not None else _read_positive_int_env("IDEALISTA_MAX_PAGES")
    ...
    def fetch(self) -> list[dict[str, Any]]:
        ...
        with httpx.Client(timeout=self.timeout_seconds, follow_redirects=True, headers=headers) as client:
            for listing_type, base_url in (
                ("buy", self.buy_search_url),
                ("rent", self.rent_search_url),
            ):
                page = 1
                while self.max_pages is None or page <= self.max_pages:
                    page_url = _set_page_query(base_url, page)
                    response = client.get(page_url)
                    if response.status_code >= 400:
                        break
                    page_items = _extract_items_from_ld_json(response.text, listing_type)
                    if not page_items:
                        break
                    new_count = 0
    ...
    def _fetch_with_playwright(self) -> list[dict[str, Any]]:
        if sync_playwright is None:
            LOGGER.warning("Idealista Playwright unavailable; falling back to static HTTP collector.")
            return []
        out: list[dict[str, Any]] = []
        try:
            with sync_playwright() as playwright:
                ...
                for listing_type, base_url in (("buy", self.buy_search_url), ("rent", self.rent_search_url)):
                    seen_urls: dict[str, None] = {}
                    stale_pages = 0
                    page_number = 1
                    while self.max_pages is None or page_number <= self.max_pages:
                        page_url = _set_page_query(base_url, page_number)
                        try:
            ...
            LOGGER.warning("Idealista Playwright collector failed, fallback to HTTP collector: %s", exc)
            return []
        return out
    ...
    for block in blocks:
        ...
        for node in _iter_nodes(payload):
            ...
            if "idealista.pt" not in url:
                continue
            if "/imovel/" not in url and "/pt/" not in url:
                continue
...
def _extract_external_id(url: str) -> str | None:
    match = re.search(r"/imovel/([0-9]+)", url)
    if match:
...
def _extract_detail_item(url: str, detail_html: str, listing_type: str) -> dict[str, Any] | None:
    ...
    bathrooms = None
    bath_match = re.search(r"([0-9]+)\s*(?:casa de banho|casas de banho|bathrooms?)", detail_html, flags=re.IGNORECASE)
    if bath_match:
        bathrooms = _safe_int(bath_match.group(1))
...
def _is_listing_url(url: str) -> bool:
    lowered = url.lower()
    if "idealista.pt" not in lowered:
        return False
    return "/imovel/" in lowered
